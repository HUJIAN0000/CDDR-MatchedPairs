...
Purpose
-------
Vectorized CPU implementation of the 5% matched-pair joint analysis. It combines 38 galaxy-cluster angular-diameter distances and their matched Pantheon+ SNe Ia with four DESI BAO D_M/r_d measurements, treating r_d as a free parameter. Cached covariance factorizations and batched linear algebra accelerate the emcee likelihood evaluation without changing the statistical model.

The code variable 'dm' denotes the linear SNe Ia absolute-magnitude evolution coefficient epsilon in M_B(z) = M_0 + epsilon z.
...
def main():
    ...
    n_data = data_cl[0].size + data_bao[0].size
    ...
    cosmo_tools.plot_getdist_advanced(
        ...
        labels,
        stats,
        filename="Pantheon_Joint_Result_GetDists_fast_cpu.pdf",
    )
