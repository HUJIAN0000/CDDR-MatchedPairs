...
Planck prior r_d = 147.09 +/- 0.26 Mpc. The affine-invariant ensemble sampler 
and likelihood are compiled with Numba; the statistical model is unchanged.
...
def load_all_data():
    ...
    if z_cl.size > 64:
        raise ValueError(
            ...
            "Use test_cluster_M_dm_eta2_fast_cpu.py instead."
        )
...
def _joint_log_prob_one(
    ...
    # Gaussian Planck prior on the sound horizon, matching the reference joint analysis
    chi2_prior = ((rd - 147.09) / 0.26) ** 2
    return cluster_log_like + bao_log_like - 0.5 * chi2_prior, cache_count
...
def ln_likelihood_reference(theta, cluster_data, bao_data):
    ...
    chi2_prior = ((rd - 147.09) / 0.26) ** 2
    return ll_cl + ll_bao - 0.5 * chi2_prior
...
def main():
    ...
    cosmo_tools.print_results(
        stats,
        lnL_best=lnL_best,
        # Cluster pairs + four BAO points + one Planck-prior constraint
        num_data=z_cl.size + z_bao.size + 1,
    )
    ...
    cosmo_tools.plot_getdist_advanced(
        ...
        labels,
        stats,
        filename="Pantheon_Joint_Result_GetDist_ultrafast_cpu.pdf",
    )
