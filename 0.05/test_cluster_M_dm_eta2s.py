...
Purpose
-------
Joint 5% matched-pair analysis combining 38 galaxy-cluster angular-diameter 
...
with r_d treated as a free parameter.
...
def load_all_data():
    ...
    cov_cl = np.loadtxt("cov_cluster_lens.txt")
...
def ln_likelihood(theta, data_cl, data_desi):
    ...
    return -0.5 * (chi2_cl + chi2_bao ) #    return -0.5 * (chi2_cl + chi2_bao )#
...
if __name__ == "__main__":
    ...
    cosmo_tools.plot_getdist_advanced(flat_samples, labels, stats, filename="Pantheon_Joint_Result_GetDists.pdf")
