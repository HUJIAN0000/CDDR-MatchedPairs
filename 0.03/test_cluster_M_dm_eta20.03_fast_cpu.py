...
Purpose
-------
Vectorized CPU implementation of the 3% matched-pair joint analysis. 
It combines 37 galaxy-cluster angular-diameter distances and their matched 
...
def load_all_data():
    ...
    cov_cl = np.loadtxt("cov_cluster_lens0.03.txt", dtype=np.float64)

    n_cl = z_cl.size
    if cov_cl.shape != (n_cl, n_cl):
        raise ValueError(
            f"cov_cluster_lens0.03.txt shape {cov_cl.shape} does not match the galaxy-cluster sample size {n_cl}."
        )
...
class FastJointLogProb:
    ...
    def log_likelihood_batch(self, theta):
        ...
        # Include the same Gaussian Planck prior on r_d as in the reference 3% joint analysis.
...
def main():
    ...
    cosmo_tools.plot_getdist_advanced(
        ...
        labels,
        stats,
        filename="Pantheon_Joint_Result_GetDist0.03_fast_cpu.pdf",
    )
