...
Purpose
-------
Cluster-only 3% matched-pair CDDR analysis using 37 galaxy-cluster angular-diameter 
distances and matched Pantheon+ SNe Ia. The script jointly constrains M_0, epsilon, 
and the CDDR-deformation parameter eta, propagating the Pantheon+ sub-covariance 
matrix and asymmetric cluster-distance uncertainties.
...
def load_data():
    ...
    try:
        ...
        print("❌ Error: file not found: matched_result_cluster0.03.csv")
        sys.exit(1)
    ...
    try:
        cov_sn = np.loadtxt("cov_cluster_lens0.03.txt")
    except FileNotFoundError:
        print("❌ Error: file not found: cov_cluster_lens0.03.txt")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    cosmo_tools.plot_getdist_advanced(flat_samples, labels, stats, filename="Cluster_Result_GetDist0.03.pdf")
